...
public:
   ...
   {
      fDebug = debug;
      if(fDebug) { std::cout << "Debugging enabled!" << std::endl; }
   }

   void AddInputFile(const char* file)
   {
      fInputFiles.emplace_back(file);
      UpdateRunNumberString();
   }
   ...
   void SetCalibration(const char* file)
   {
      delete fCalibration;
      fCalibration = new class Calibration(file);
   }
