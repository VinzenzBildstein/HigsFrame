...
template <class T>
class Singleton : public TObject {   // NOLINT(cppcoreguidelines-virtual-class-destructor)
public:
...
protected:
   ...
   ~Singleton() = default;   // NOLINT(cppcoreguidelines-virtual-class-destructor)
...
private:
   ...
   /// \cond CLASSIMP
   ClassDef(Singleton, 1)   // NOLINT(readability-else-after-return)
   /// \endcond
};

/// \cond CLASSIMP
templateClassImp(Singleton)
/// \endcond

template <class T>
T* Singleton<T>::fSingleton = nullptr;

template <class T>
TDirectory* Singleton<T>::fDir = nullptr;

template <class T>
void Singleton<T>::Streamer(TBuffer& R__b)
{
